...
test_that("pmm works for logical y", {
  expect_equal(
    pmm(1, xtest = Sys.Date(), ytrain = TRUE), 
    TRUE
  )
})
...
test_that("pmm works for factor y", {
  expect_equal(
    pmm(1, xtest = Sys.Date(), ytrain = factor("A")), 
    factor("A")
  )
})
...
test_that("pmm works for date y", {
  expect_equal(
    pmm(1, xtest = Sys.Date(), ytrain = Sys.Date()), 
    Sys.Date()
  )
})
...
test_that("pmm works with NA in xtrain", {
  expect_equal(
    pmm(c(NA, 1, 2), 1, ytrain = c(0, 1, NA)), 
    1
  )
})
...
test_that("pmm works with NA in xtrain and NA in ytrain", {
  expect_equal(
    pmm(c(NA, 1, 2), 1, ytrain = c(0, NA, 3)), 
    3
  )
})
